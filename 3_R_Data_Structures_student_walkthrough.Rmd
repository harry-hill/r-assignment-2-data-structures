...
```{r}
# if you try to teach it just like a list in Python you will have a headache
...
```{r}
# similar to how we add lists in pandas but this is default!
...
```{r}
# R easily moves between integer and float data types
...
# in python: [[94,96]],["John", "Jake"], 6]

# very intuitive way of indexing
student_list[[2]]
```
...
```{r}
# similar to python dataframes but have some important differences
# row having unique name =~= python & sql primary keys
...
```{r}
# each column is like a 'variable' and each item is like an 'observation'
  # very science-framed language
str(df)
```
...
# comma to say 'I don't want to see anything else'
df[1:2,]
```
...
```{r}
# counts down columns then back up to the next in row
my_matrix <- matrix(
...
my_matrix <- matrix(
  ...
  ncol <- 5,
  byrow <- FALSE,
  dimnames <- list(c("ROW 1", "ROW 2"), c("COL 1", "COL 2", "COL3", "COL 4", "COL 5"))
)
...
```{r}
# okayyy middle school pre-algebra
...
```{r}
# builds array down the column then back up to next in top row
# will recycle values from the data vector when it gets to the end
# commas are where the dimnames will go 
my_array <- array(
  # data
...
```{r}
# level designates a custom order for sorting discrete values
my_factor <- factor(
...
my_array <- array(
  # bind together two matrices into array
  c(first_matrix,second_matrix),
  # dimensions; 3 rows, 3 cols, 2 matrices
  dim <- c(3,3,2),
  # row/column/matrix names
  dimnames <- list(
    c("ROW 1", "ROW 2", "ROW 3"),
    c("COL 1", "COL 2", "COL 3"),
    c("MATRIX 1", "MATRIX 2"))
...
```{r}
# ',' in place of index position says 'all of'
# 3rd row, all columns, 1st matrix
my_array[3,,1]
```

2.  Print the second column of both matrices.

    ```{r}
    # all rows, 2nd column, all matrices
    my_array[,2,]

    # all rows, 2nd column, 1st-2nd matrices
    my_array[,2,(1:2)]
